...
model01 = load_model('/app/textauthor1/model_author_all.h5')
with open('/app/textauthor1/tokenizer.pickle', 'rb') as handle:
    tokenizer2 = pickle.load(handle)
pictures_path = '/app/textauthor1/pictures/'
...
st.image(pictures_path+'pipelineathors2.jpg',caption='Этапы проектирования кейса')
with st.expander("Общая схема"):
    st.markdown(
        ...
        \nОни нужны для преобразования текстовой информации в числовую. Обычный текст превращается в матрицу, состоящую из чисел.
        \n4. Обучение сети:
        ...
        \nИз датасета по каждому автору выделяется небольшой отрывок произведения и подается в сеть. Их задача преобразовать эти тексты в матрицы согласно частотному словарю.
        \n6. Сравнение результатов:
        \nМожно по очереди вводить в сеть отрывки текстов и смотреть как сеть их распознает. Внимание! В тестовой выборке могут присутствовать фрагменты из текстов, с которыми сеть не знакома. В этом случае она распределит проценты вероятности на всех известных ей авторов.
        \n7. Приложение Streamlit:
